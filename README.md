...
薄盘 ETF 在流动性差的时段会退化成低频源，这与 Chainlink 是同类问题，只是触发机制不同
（Chainlink 靠 deviation 阈值，3030 靠有没有人报价）。实测同一交易日下午：

| | 挂单量(bid/ask) | spread 中位 | 报价更新间隔 p90 / 最长 |
|---|---|---|---|
| 13:00–13:46 | 80,000 / 80,000 股 | 28.5 bps | 2.3 分 / 3.2 分 |
| 13:46 之后 | **10,000 / 30,000 股** | 14.3 bps | **10.7 分 / 36.0 分** |

注意 spread 反而收窄——那不是做市更积极，是深度掉了一个数量级后小单容易挂窄。
该时段 3030 有 62.5% 的时间钉在同一价位。

**这会在短采样间隔上制造 beta 衰减的假象**，必须用多个时间尺度交叉验证：

| 采样间隔 | 13:46 前 beta | 13:46 后 beta |
|---|---|---|
| 15 秒 | 1.014 | 0.255 |
| 1 分钟 | 0.968 | 0.387 |
| 5 分钟 | 0.924 | 0.456 |
...
beta 随间隔单调回升、且基差水平无趋势漂移（首尾 −4.0 → −4.1 bps）、
端到端位移一致（Pyth −14.5bps vs 3030 −14.3bps）——三者共同说明**它仍然跟随金价，
只是路径是台阶而非曲线**，并非脱钩。

看板的"报价陈旧度对比"图把 Chainlink 与 3030 画在同一轴上，正是为了让这两种陈旧可比。

### 绝对价格视图

看板第一张图把所有源画在同一条 **USD/oz** 纵轴上。这是判断"谁在动"最直观的视角：
Chainlink 在两次链上更新之间是一条**水平线**，其余六条同步跟随金价起伏；
曲线之间的垂直距离就是基差。

该图的各源绝对价由 `P_i = pyth × (1 + d_i/1e4)` 在前端反算，**不额外传输任何数据**。
